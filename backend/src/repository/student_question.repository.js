...
async function get_question_details_for_user(question_id, user_id) {
  ...
  if (question && question.answers) {
    ...
    // Add reaction summaries and user reaction status to each answer
...
module.exports = {
  ...
  get_question_details: async (question_id) => {
    ...
    if (question && question.answers) {
      ...
      // Add reaction summaries to each answer
      question.answers.forEach(answer => {
        if (answer.reactions && answer.reactions.length > 0) {
          answer.reactionSummary = answer.reactions.reduce((acc, reaction) => {
            acc[reaction.emoji] = (acc[reaction.emoji] || 0) + 1;
  ...
  react_to_answer: async (question_id, user_id, answer_student_id, emoji) => {
    ...
    const answerIndex = q.answers.findIndex(a => String(a.student_id) === String(answer_student_id));
    ...
    return { 
      success: true, 
      reactionSummary,
      userReaction: answer.reactions.find(r => String(r.user_id) === String(user_id))?.emoji || null
    };
  },
